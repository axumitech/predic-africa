...
function Content(props: Props) {
    ...
    if (page === 'positions')
        return <Panel title="Votre carnet de positions"><p>Les cotes sont verrouillées à l’achat. Le retour potentiel inclut votre mise. Les positions restent engagées jusqu’à la résolution ; il n’y a pas de revente dans ce mode.</p>{!props.positions?.data.length ? <Empty>Vous n’avez pas encore de position. <Link href="/markets">Explorer les marchés →</Link></Empty> : <div className="p-table-wrap table-responsive"><table className="data-table"><thead><tr><th>Marché</th><th>Choix</th><th>Mise</th><th>Cote</th><th>Retour potentiel</th><th>Statut</th></tr></thead><tbody>{props.positions.data.map(p => <tr key={p.id}><td><Link href={`/markets/${p.market_id}`}>{p.title}</Link></td><td>{p.outcome === 'yes' ? 'Oui' : 'Non'}</td><td>{money(p.stake)}</td><td>×{(p.odds / 100).toFixed(2)}</td><td>{money(p.payout)}</td><td><Status value={p.status}/></td></tr>)}</tbody></table></div>}<Pager result={props.positions}/></Panel>;
...
}) { return <Panel title="Journal de vos opérations" className="table-container">{!result?.data.length ? <Empty>Aucune transaction. <Link href="/wallet">Ajouter des crédits de démonstration →</Link></Empty> : <div className="p-table-wrap table-responsive"><table className="data-table"><thead><tr><th>Date</th><th>Opération</th><th>Crédits</th><th>Référence</th></tr></thead><tbody>{result.data.map(t => <tr key={t.id}><td>{date(t.created_at)}</td><td>{t.kind}</td><td>{['Mise', 'Retrait simulé'].includes(t.kind) ? '−' : '+'}{money(t.amount)}</td><td><code>{t.reference}</code></td></tr>)}</tbody></table></div>}<Pager result={result}/></Panel>; }
